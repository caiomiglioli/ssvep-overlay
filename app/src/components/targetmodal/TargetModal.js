...
export default function TargetModal({ params }) {
  ...
  useEffect(() => {
    setFormValues({
      posX:
        params.mode === "new"
          ? parseFloat(((params.event.clientX / window.innerWidth) * 100).toFixed(1))
          : params.target.posX,
      posY:
        params.mode === "new"
          ? parseFloat(((params.event.clientY / window.innerHeight) * 100).toFixed(1))
          : params.target.posY,
    ...
    formValues && (
      <dialog open={params.open} className="modal">
        <div className="modal-box">
          ...
          <h3 className="font-bold text-lg">{params.mode === "new" ? "Criar Target" : "Editar Target"}</h3>
