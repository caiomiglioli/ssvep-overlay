...
function openTargetsWindow(window) {
  ...
  if (path) {
    ...
    if (!file.overlay) {
      return dialog.showErrorBox('Erro ao abrir arquivo de configuração.', `O arquivo ${path[0].split('/').pop()} não é um arquivo de configuração válido.`);
    }

    createTargetsWindow(file.targets, path[0])
  }
}
...
function createMainWindow() {
  ...
  // ---------------- ipc on
  ipcMain.handle('overlay:new', () => createTargetsWindow({}))
